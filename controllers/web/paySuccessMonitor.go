...
func (p *PaySuccessMonitor) RunSendNotice(message string) {
    ...
    //}
    go func() {
        resp, err := http.Get(fmt.Sprintf("http://voice.arch800.com/notice/singleCallByTts?system=Monitoring&type=voice&errorMsg=%s", url.QueryEscape(message)))
        if err != nil {
            fmt.Println(err)
            return
        }
